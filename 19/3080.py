...
def m(h): # Эта функция генерит все возможные ходы из позиции, которую в неё передали (позиция это число)
...
def g(h): # тут сама логика задачи, на вход она принимает позицию (сколько сейчас камней в куче) и возвращает исход игры при этой позиции
    if h > 37: # если кол-во камней больше чем нужно для победы, то мы победили
        return 'w'
